...
--
-- Host: localhost:3306
-- Generation Time: May 04, 2017 at 03:08 PM
...
(101, 'bacon strips', 9.95, 1, 0),
(102, 'fish n chips', 7.99, 1, 0),
(103, 'beef on a stick', 7.77, 0, 0),
(105, 'jelly pockets', 4.5, 1, 0),
(106, 'frog paste', 9.95, 1, 66);
...
--
-- Stand-in structure for view `viewnamemostbids`
--
CREATE TABLE IF NOT EXISTS `viewnamemostbids` (
`id` int(11)
,`name` varchar(65)
,`bids` int(11)
);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`innova18`@`localhost` SQL SECURITY DEFINER VIEW `mailing` AS (select concat(`customers`.`city`,', ',`customers`.`state`) AS `address` from `customers`);

-- --------------------------------------------------------

--
-- Structure for view `viewnamemostbids`
--
DROP TABLE IF EXISTS `viewnamemostbids`;

CREATE ALGORITHM=UNDEFINED DEFINER=`innova18`@`localhost` SQL SECURITY DEFINER VIEW `viewnamemostbids` AS select `items`.`id` AS `id`,`items`.`name` AS `name`,`items`.`bids` AS `bids` from `items` order by `items`.`bids` desc limit 10;
