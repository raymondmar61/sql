...
*/
--RM:  Page 761 table differentiate Physical Window and Logical Window for which I don't understand.
select numeric_grade, grade_type_code, grade_code_occurrence as occur, avg(numeric_grade) over (order by grade_type_code desc rows between unbounded preceding and current row) as "moving grade type average desc", avg(numeric_grade) over (order by numeric_grade desc rows between unbounded preceding and current row) as "moving grade average desc"
from grade
order by grade_type_code desc;  --sort order by grade_type_code.  Reading the moving grade averages ok for grade_type_code, not ok for numeric_grade.
...
SELECT numeric_grade, grade_type_code, grade_code_occurrence AS occur, AVG(numeric_grade) OVER(ORDER BY student_id, section_id, grade_type_code,
grade_code_occurrence) AS cumavg_over_pk, AVG(numeric_grade) OVER(PARTITION BY grade_type_code ORDER BY student_id, section_id, grade_type_code, grade_code_occurrence) AS partition
FROM grade
WHERE student_id = 254
AND section_id = 87;
--The LAG and LEAD functions allow you to get values from other rows relative to the position of the current row. The syntax is as follows: {LAG|LEAD}(expression[,offset][,default]) OVER ([query_partition_clause] order_by_clause).  The LAG function returns one of the values of the previous rows, and the LEAD function returns one of the values of the next rows. The optional OFFSET parameter identifies the relative position of the row; if no parameter is specified, it defaults to 1. The optional default parameter returns the value if the offset falls outside the boundaries of the table or the partition, such as the last and first rows. The LAG and LEAD functions do not have a windowing clause because the offset indicates the exact row.
SELECT TO_CHAR(start_date_time, 'MM') "Month", SUM(cost) "This Month's Revenue", LAG(SUM(cost),1) OVER (ORDER BY TO_CHAR(start_date_time, 'MM'))
"Previous Month", SUM(cost)-LAG(SUM(cost),1) OVER (ORDER BY TO_CHAR(start_date_time, 'MM')) "Monthly Change"
FROM enrollment e, section s, course c
WHERE e.section_id = s.section_id
AND s.course_no = c.course_no
AND c.cost IS NOT NULL
GROUP BY TO_CHAR(start_date_time, 'MM');
select course_no, revenue, num_of_sections, avg(revenue) over (partition by num_of_sections) as "avg rev per section"
from course_revenue;  --RM:  I don't have course_revenue table uploaded
/*
...
WHERE revenue > (SELECT AVG(revenue)
	FROM revenue_per_instructor);  --The WITH clause creates a name for the "temporary" result REVENUE_PER_INSTRUCTOR.  This result is then referred to in the subsequent SELECT statements in the context of the original query.
SELECT instructor_id, SUM(cost)
FROM section s, course c, enrollment e
WHERE s.section_id = e.section_id
AND c.course_no = s.course_no
GROUP BY instructor_id;  --calculate the sum of the cost for each instructor
SELECT avg(SUM(cost))
FROM section s, course c, enrollment e
WHERE s.section_id = e.section_id
AND c.course_no = s.course_no
GROUP BY instructor_id;  --calculate the average sum of all costs for each instructor
SELECT *
FROM (SELECT instructor_id, SUM(cost) AS revenue, AVG(SUM(cost)) OVER() AS avg
	FROM section s, course c, enrollment e
	WHERE s.section_id = e.section_id
	AND c.course_no = s.course_no
	GROUP BY instructor_id) t
WHERE revenue > avg;
--Oracle can perform spreadsheet-like calculations in queries. This capability can be useful for budgeting and forecasting. It allows the display of additional rows or calculations in the query result through the application of formulas in the MODEL clause of a SELECT statement.
select *
from model_example
order by year, gender, course;
select course, gender, year, partition
from model_example
	model partition by (gender)
	dimension by (year, course)
	measures (enroll_no partition)
	(partition[2009,'Spanish II'] = partition[2008, 'Spanish I'], partition[2010,'Spanish III'] = round((partition[2009,'Spanish II'])*0.80))
order by year, gender, course;
/* the GENDER column is the partitioned column, and the DIMENSION columns YEAR and COURSE identify the groups within each partition. The MEASURE column is ENROLL_NO and is uniquely identified by the combination of partition and dimension columns.

The rule s[2009, ‘Spanish II’] = s[2008, ‘Spanish I’] is a definition of an assignment. A rule consists of references. The two-dimensional [2009, ‘Spanish II’] reference defines a single cell for the dimensions YEAR and COURSE. The measure partiaion is an alias, as defined in the MEASURE
element. The left side of the assignment indicates the destination cell; the right side determines the values for it. In this instance, the destination cell with the year 2009 and the course Spanish II does not exist in the MODEL_EXAMPLE table and is created in the result set by default; this action is referred to as UPSERT. The right side of the assignment states that the measure cell should be the same value as the value in year 2008 and for the course Spanish I. Two rows are created for this year: one for male and another for female.

The next rule, s[2010, ‘Spanish III’] = ROUND((s[2009, ‘Spanish II’])*0.80), defines that the enrollment number for Spanish III in year 2010 should be 80 percent of the enrollment for Spanish II in 2009, rounded to nearest whole number.
*/
/*
The following example illustrates the projection of enrollment figures for Spanish II for the years 2009 through 2011 (shaded in gray). The FOR
loop creates these years. The measure values are based on the previous year’s value of Spanish II. The CURRENTV( ) function returns the current value of the YEAR dimension column, but because the function reads CURRENTV( )-1, it looks at the previous year’s value. The calculation ROUND((s[CURRENTV( )-1, ‘Spanish II’])*1.1) assumes a 10 percent increase each subsequent year and is rounded to the nearest whole number.
*/
SELECT course, gender, year, partition
FROM model_example
WHERE course = 'Spanish II'
	...
	DIMENSION BY (year, course)
	MEASURES (enroll_no partition)
	(partition[FOR year FROM 2009 TO 2011 INCREMENT 1,'Spanish II'] = ROUND((partition[CURRENTV()-1,'Spanish II'])*1.1))
ORDER BY year, gender, course;
/*
...
select gender, semester_year as year, semester_month as month, sum(num_of_students) as total
from instructor_summary
group by rollup(gender, semester_year, semester_month);  --the formation of subtotals for each of the groups using rollup.  You needed only one query to generate different groupings of data. The individual groupings are group 1: gender, year, and month; group 2: gender and year; group 3: gender; and group 4: grand total.
--The CUBE operator takes the formation of super aggregates another step further: It allows you to generate all the possible combinations of groups.
select gender, semester_year as year, semester_month as month, sum(num_of_students) as total
from instructor_summary
